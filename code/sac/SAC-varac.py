...
class SAC(object):
    ...
    def learn(self, relay_buffer):
        ...
        # transfer the reward
        if replay_buffer.count < 10000:
            y = np.mean(replay_buffer.r[-10000:])
        else:
            y = np.mean(replay_buffer.r[replay_buffer.count-10000:replay_buffer.count])
        batch_r_ = batch_r - 0.3 * batch_r ** 2 + 0.6 * batch_r * y

        with torch.no_grad():
            batch_a_, log_pi_ = self.actor(batch_s_)  # a' from the current policy
            # Compute target Q
            target_Q1, target_Q2 = self.critic_target(batch_s_, batch_a_)
            target_Q = batch_r_ + self.GAMMA * (1 - batch_dw) * (torch.min(target_Q1, target_Q2) - self.alpha * log_pi_)
...
def evaluate_policy(env, agent):
    times = 40  # Perform  evaluations and calculate the average reward and the step-wise variance
    evaluate_reward = []
    ...
    return int(np.sum(evaluate_reward) / times), np.var(evaluate_reward)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--number", type=int, default=0, help="mark the mission index")
    parser.add_argument("--seed", type=int, default=0, help="set the random seed")
    parser.add_argument("--env_id", type=int, default=0, help="set the environment index")
    parser.add_argument("--T", type=int, default=20, help="set the max training steps")
    parser.add_argument("--F", type=int, default=1, help="set the evaluating frequence")
    args = parser.parse_args()

    env_name = ['Pendulum-v1', 'BipedalWalkerHardcore-v3', 'HalfCheetah-v2', 'Hopper-v2', 'Walker2d-v2']
    ...
    max_train_steps = args.T * 1e5  # Maximum number of training steps
    random_steps = 25e3  # Take the random actions in the beginning for the better exploration
    evaluate_freq = args.F * 5e3  # Evaluate the policy every 'evaluate_freq' steps
    evaluate_num = 0  # Record the number of evaluations
    evaluate_rewards = []  # Record the rewards during the evaluating
    evaluate_variances = [] # Record the step-wise vairance during the evaluating
    ...
    while total_steps < max_train_steps:
        ...
        while not done:
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                evaluate_num += 1
                evaluate_reward,evaluate_variance = evaluate_policy(env_evaluate, agent)
                evaluate_rewards.append(evaluate_reward)
                evaluate_variances.append(evaluate_variance)
                print("evaluate_num:{} \t evaluate_reward:{} \t evaluate_variance:{}".format(evaluate_num, evaluate_reward, evaluate_variance))
